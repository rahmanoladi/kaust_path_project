...
def train(data_loader, device, model, loss_function, optimizer):
...
@torch.no_grad()
def test(data_loader, device, model, loss_function):
...
def define_model(trial):
    model = deeper_gcn_model(args.use_path_info, args.num_layers, args.inp_dim, args.edge_dim, args.hidden_dim, args.num_classes, args.dropout,  args.max_len, pooling='mean')
...
def objective(trial):
    ...
    args.hidden_dim = trial.suggest_int("hidden_dim", 1, args.max_hidden_dim)
    ...
    for epoch in range(0, args.epochs): 
        ...
        train_loss, train_score = train(train_loader, device, model, loss_fn, opt) 
        val_loss, val_score =   test(val_loader, device, model, loss_fn)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_hidden_dim', type=int, default= None, help='hidden layer(s) dimension')
