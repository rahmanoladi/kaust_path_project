...
def normalize_adj(g, max_nodes):
    ...
    adj = torch.eye(max_nodes, max_nodes) + adj
    
    degs = g.in_degrees()    
    degs = torch.reshape(degs, (degs.shape[0], 1) )
    degs = split_pad_and_stack_feat(degs, batch_num_nodes, max_nodes)   
    degs = torch.from_numpy(degs).float()
    degs = degs + 1.0
    degs = 1.0/torch.sqrt(degs)
    degs = torch.matmul(degs, torch.permute(degs, (0, 2, 1)))

    adj = torch.matmul(torch.matmul(degs, adj), degs)
    return adj
...
def path_vs_no_path(list_no_path, list_path, order):

    len_no_path = len(list_no_path)
    len_path = len(list_path)
    count_no_path = 0
    count_path = 0
    for i in range(len_no_path):
        for j in range(len_path):
          if order == 'lower':
            if list_no_path[i]< list_path[j]:
                count_no_path = count_no_path + 1
            elif list_path[j]<list_no_path[i]:
    ...
    out = (count_path - count_no_path)/(len_no_path*len_path)
    return (count_no_path, count_path, out)


def mean_and_std_of_trials(list_no_path, list_path, dataset_name, method_name):
    print('\n ', method_name, ' on ', dataset_name,': ' )
    print('mean and std of without path: ', np.mean(np.asarray(list_no_path)), np.std(np.asarray(list_no_path)))
    print('mean and std of with path: ', np.mean(np.asarray(list_path)), np.std(np.asarray(list_path)))


list_no_path = [1.087, 1.162, 1.087]
list_path = [0.992, 1.113, 0.994]
#print("path_vs_no_path:", path_vs_no_path(list_path, list_no_path, 'lower'))
#print("ESOL: ", np.mean(np.asarray(list_no_path)), np.mean(np.asarray(list_path)))

list_no_path = [0.692, 0.696, 0.705]
list_path = [0.673,0.685,0.701]
#print("path_vs_no_path:", path_vs_no_path(list_path, list_no_path, 'higher'))
#print("BBBP: ", np.mean(np.asarray(list_no_path)), np.mean(np.asarray(list_path)))




list_no_path = [4.582,3.393,3.00]
list_path = [3.055,3.969,3.237]
#print("path_vs_no_path:", path_vs_no_path(list_path, list_no_path, 'lower'))
#print("FreeSolv: ", np.mean(np.asarray(list_no_path)), np.mean(np.asarray(list_path)))

list_no_path = [0.755, 0.771, 0.782]
list_path = [0.732, 0.763, 0.722]
#print("path_vs_no_path:", path_vs_no_path(list_path, list_no_path, 'higher'))
#print("BACE: ", np.mean(np.asarray(list_no_path)), np.mean(np.asarray(list_path)))
   

     

list_no_path = [0.612,0.605,0.602]
list_path = [0.567,0.661,0.573]
#print("ClinTox: ", np.mean(np.asarray(list_no_path)), np.mean(np.asarray(list_path)))
...
def unbatch_adj_tensor(adj, batch_num_nodes):
    index_list = batch_num_nodes_to_index(batch_num_nodes)
    batch_size = adj.shape[0]
    total_nodes = sum(batch_num_nodes)
    out = torch.zeros(total_nodes, total_nodes)

    for i in range(batch_size):
        out[index_list[i]:index_list[i+1], index_list[i]:index_list[i+1]] = adj[i, 0:batch_num_nodes[i], 0:batch_num_nodes[i]]
    return out



def add_noise_to_node_0(dgl_graph, seed, node_mean, node_std):
...
def add_noise_to_node_and_edge(dgl_graph, seed, node_std, edge_std):
        torch.manual_seed(seed)
        node_feats = dgl_graph.ndata['h']
        node_noise = torch.normal(mean=0.0, std = node_std)
        dgl_graph.ndata['h'] = node_feats + node_noise 
        ...
        edge_noise = torch.normal(mean=0.0, std = edge_std)
        dgl_graph.edata['e'] = edge_feats + edge_noise 
        #print("dgl_graph.edata['e'].shape: ", dgl_graph.edata['e'].shape)
        return dgl_graph
...
def node_and_edge_mean_and_std(dataset, DataLoader, collate_fn, num_workers):
