...
from utils import count_files, max_nodes_and_edges, max_path_len, early_stopping, beta_mat_3d_from_dgl_graph, split_mat_3d, pad_mat_3d, split_mat_2d, pad_mat_2d, split_pad_and_stack_beta, split_pad_and_stack_adj, split_pad_and_stack_feat
...
def train(data_loader, num_batches, device, max_nodes, max_path_len, model, loss_function, optimizer):
...
@torch.no_grad()
def test(data_loader, num_batches, device, max_nodes, max_path_len, model, loss_function):
    ...
    for i, batch in enumerate(data_loader):
        ...
        try:
            ...
            if args.add_noise:
                dgl_graph = add_noise_to_node_and_edge(dgl_graph, num_batches * args.run_index +  i, args.node_std, args.edge_std)
            edge_feats = dgl_graph.edata['e'] 
            #print("edge_feats.shape in test: ", edge_feats.shape)

        except KeyError: 
            #print("continue in test")    
            continue
...
def main():
    ...
    model = graphormer_model(args.use_path_info, args.num_layers, inp_dim, edge_dim, args.hidden_dim, args.num_heads, args.ff_dim, args.num_classes, args.max_len, args.dropout)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--small_hidden_dim', type=int, default= 100, help='small hidden dimension for attention. we multiply this by number of heads to get embedding size.')
    ...
    parser.add_argument('-me', '--metric', choices=['roc_auc_score', 'pr_auc_score', 'r2', 'mae', 'rmse'], default='rmse', help='Metric for evaluation (default: roc_auc_score)')
    ...
    parser.add_argument('--dir_to_save_model', type=str, default= '/ibex/user/ibraheao/path_project/graphormer/model_directory_2', help='power dimension + 1')
